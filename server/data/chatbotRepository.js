  ...
  try {
    ...
    const userActivities = await pool.request()
      ...
      .query(`
        ...
        LEFT JOIN [dbo].[Employee] e ON a.UserID = e.UserID
    ...
    if (userActivities.recordset.length) {
      results.myTasks = userActivities.recordset.map(task => ({
        ...task,
        isOverdue: !task.Status && new Date(task.DueDate) < new Date(),
    ...
    if (queryLower.includes("deal") || queryLower.includes("pipeline") || 
      ...
      const myDeals = await pool.request()
        ...
        .query(`
          ...
          WHERE d.UserID = @UserId AND d.Active = 1
        `);
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        ...
        LEFT JOIN [dbo].[JobTitle] jt ON e.JobTitleID = jt.JobTitleID
        WHERE u.UserID = @UserId AND u.Active = 1 AND e.Active = 1
      `);
...
async function getBusinessInsights(userId) {
  try {
    ...
    // Deal pipeline analysis by stage
    ...
    const topAccounts = await pool.request()
      ...
      .query(`
        ...
        FROM [dbo].[Account] a
        LEFT JOIN [dbo].[Deal] d ON a.AccountID = d.AccountID
        LEFT JOIN [dbo].[Industry] i ON a.IndustryID = i.IndustryID
        WHERE d.UserID = @UserId AND a.Active = 1
...
async function getEmailContext(userQuery, userId) {
  try {
    ...
    // Try to find mentioned contact or account
